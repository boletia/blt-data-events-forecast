...
def main():
    ...
    if utils.DEPLOY_LOCAL:
        ...
        st.write("Se están usando modelos en modo local *")
    else:
        # use sagemaker client 
        client = get_deploy_client(f"sagemaker:/us-east-1")
        st.write("Se están usando modelos via sagemaker API *")


    st.title("Forecast de venta de boletos")
    ...
    if st.button("Obtener Predicción"):
        ...
        # If the artist data is not complete, then use the 'simple model'
        if ig_followers == 0 or spotify_followers == 0:

            st.write("Predicción obtenida mediante un modelo sin métricas de artista *")
            
            # Prepare the payload for the simple model
            numeric_data = pd.DataFrame({
                "LEAD_TIME_DAYS": [lead_time_days],
                "START_WEEK": [start_week],
                "START_DAY": [start_day],
            ...
            # Normalize numeric data
            normalized_numeric_data = utils.normalize(numeric_data, 'scaler2.pkl')
            normalized_numeric_values = normalized_numeric_data.tolist()[0]
            ...
            if utils.DEPLOY_LOCAL:
                ...
                columns = payload_data["dataframe_split"]["columns"]
                data = payload_data["dataframe_split"]["data"]
                df = pd.DataFrame(data, columns=columns)
                # Make the prediction using local simple model
                tickets = simple_model.predict(df)[0]

            else:
                # Prepare payload for API model
                payload = json.dumps(payload_data)
                # Use API sagemaker simple model
                prediction = client.predict('ticketssimplemodel', payload)
                # Make prediction from API
                tickets = prediction['predictions'][0]

            margin_of_error = utils.margin_of_error_simple_model


        # If the artist data is complete then use the regular model
        else:
            ...
            if utils.DEPLOY_LOCAL:
                ...
                data = payload_data["dataframe_split"]["data"]
                df = pd.DataFrame(data, columns=columns)
                # Make the prediction using local model
                tickets = model.predict(df)[0]
            ...
            margin_of_error = utils.margin_of_error_regular_model

        # Calculate the range of the prediction
        lower_bound = max(0, min(tickets - margin_of_error, ticket_type_quantity))
        upper_bound = max(0, min(tickets + margin_of_error, ticket_type_quantity))

        # Calculate the range value of the rest of the metrics
        tickets_value_lower = lower_bound * ticket_type_price
        tickets_value_upper = upper_bound * ticket_type_price
        soldout_pct_lower = lower_bound / ticket_type_quantity * 100
        soldout_pct_upper = upper_bound / ticket_type_quantity * 100

        # Show the predictions with confidence intervals
        st.write("\n\n")
        #st.write("Prediccion central:", tickets)
        st.metric("Valor aproximado de los tickets:", f" entre ${prettify(round(tickets_value_lower))} y ${prettify(round(tickets_value_upper))} MXN")
        st.metric("Tickets vendidos aproximados:", f" entre {prettify(round(lower_bound))} y {prettify(round(upper_bound))} tickets")
        st.metric("Porcentaje de sold out:", f" entre {round(soldout_pct_lower, 2)}% y {round(soldout_pct_upper, 2)}%")
